...
import {empTimeOut} from '../api/authenticationService';
...
const EmployeeTimeout=({loading,error,...props})=>{
    ...
    const handleSubmit=(evt)=>{
        ...
        empTimeOut(values).then((response)=>{

            console.log("response",response);
            if(response.status===200){
                //console.log(response.data);
                props.setUser(response.data);
                navigate('/dashboard/timein');
            }
            else{
...
export default connect(mapStateToProps,mapDispatchToProps)(EmployeeTimeout);